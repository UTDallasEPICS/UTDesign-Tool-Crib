...
export default withPageAuthRequired(function BorrowTool() {
    const {data: teamData, isLoading: teamIsLoading} = useSWR("/api/teams", fetcher)
    const {data: toolData, isLoading: toolIsLoading} = useSWR("/api/tools", fetcher)
    ...
    const refactorTeamData = (num) => {
      ...
      if (num > 0) {
        ...
        const currentTeams = teamData.data.filter((item) => item.teamNumber == num);
    ...
    return(
        ...
        <div className="input-box">
          ...
          /> */}
          <DatePicker clearIcon={null} onChange={setDueDate} value={dueDate}/>
          {/* <Link href="/"> */}
            <button disabled={!(currentToolLimit > 0 && toolId > 0 && teamId > 0 && memberId > 0 )}
              className="submit"
              onClick={() => {
                submitLogEvent();
              }}
