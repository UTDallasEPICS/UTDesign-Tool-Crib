...
export default function Dashboard() {
  ...
  const {data: logData, mutate, error, isLoading} = useSWR("/api/logs/current", fetcher, { refreshInterval: 300000}); // Refresh every 5 minutes automatically
            ...
            logData.data.map((item) =>
              (new Date(item.dateDue) < loadTime) ? (
                <div id="late" className="column-grid" key={item.id}>
                  ...
                  <div className="cell">
                    <p style={{ color: "red" }}>{new Date(item.dateDue).toLocaleDateString()}</p>
                  </div>
                  <div className="cell">
                ...
                <div className="column-grid" key={item.id}>
                  ...
                  <div className="cell">{new Date(item.dateDue).toLocaleDateString()}</div>
